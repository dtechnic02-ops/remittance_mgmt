<x-app-layout>
    <x-slot name="header"><div><h2 class="text-xl font-semibold text-gray-800">Expense Import Preview</h2><p class="mt-1 text-sm text-gray-500">All rows are valid. Confirm to post through the normal Expense service.</p></div></x-slot>
    <div class="py-8"><div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8"><div class="overflow-hidden rounded-lg bg-white shadow-sm">
        ...
        </tbody></table></div><div class="flex justify-end gap-3 border-t p-5"><a href="{{ route('expenses.import.create') }}" class="rounded-md border px-4 py-2 text-sm">Back</a><form method="POST" action="{{ route('expenses.import.confirm') }}">@csrf<button class="rounded-md bg-gray-900 px-5 py-2 text-sm font-medium text-white">Confirm Import</button></form></div>
    </div></div></div>
</x-app-layout>
