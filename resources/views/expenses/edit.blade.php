    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('expenses.show', $expense) }}"
               class="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Back</a>
        </div>
    </x-slot>
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm">
                <div class="border-b border-gray-200 p-5">
                    <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                        <div>
                            <div class="text-xs font-medium uppercase text-gray-500">Expense Category</div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 font-semibold text-gray-900">{{ number_format($expense->amount) }}</div>
                        </div>
                    </div>
                </div>

                <form method="POST" action="{{ route('expenses.update', $expense) }}" class="p-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input id="date_ad" type="date" name="date_ad"
                                   value="{{ old('date_ad', $expense->date_ad?->format('Y-m-d')) }}" required
                        ...
                        <div class="grid gap-4 rounded-lg bg-gray-50 p-4 sm:grid-cols-2">
                            <div>
                                <label for="date_bs" class="text-xs font-medium uppercase text-gray-500">Date BS</label>
                                <input id="date_bs" type="text" readonly value="{{ $expense->date_bs }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 font-semibold text-gray-900 shadow-sm">
                            </div>
                            <div>
                                <label for="financial_year" class="text-xs font-medium uppercase text-gray-500">Financial Year</label>
                                <input id="financial_year" type="text" readonly value="{{ $expense->financial_year }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 font-semibold text-gray-900 shadow-sm">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="note" name="note" rows="5" maxlength="2000"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('note', $expense->note) }}</textarea>
                        </div>

                        <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
                            Only <strong>Date AD</strong> and <strong>Note</strong> can be edited. Expense number,
                            category, account and amount cannot be changed.
                        </div>

                        <div class="flex justify-end gap-3">
                            <a href="{{ route('expenses.show', $expense) }}"
                               class="rounded-md border border-gray-300 px-5 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</a>
                            <button type="submit"
                                    class="rounded-md bg-gray-800 px-5 py-2 text-sm font-semibold text-white hover:bg-gray-700">Save Changes</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            async function updateNepaliDate() {
                ...
                try {
                    ...
                    if (! response.ok) throw new Error('Date conversion failed.');
